(function(){
  ...
  function _validStageSource(source){
  ...
  function buildManualStage(dateISO,stages,current,nowTs){
    ...
    for(var i=dates.length-1;i>=0;i--){
      if(dates[i]<dateISO){anchor=_validStageSource(stages[dates[i]]);break;}
    }
    if(!anchor){
      for(var j=0;j<dates.length;j++){
        if(dates[j]>dateISO){anchor=_validStageSource(stages[dates[j]]);break;}
      }
    }
    if(!anchor)anchor=_validStageSource(current);
